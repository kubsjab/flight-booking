...
object FlightGenerator {
  ...
  def apply(airlineManager: ActorRef[AirlineManager.Command]): Behavior[Command] = Behaviors.withTimers(timers =>
    Behaviors.receive { (context, message: Command) =>
      ...
      message match {
        ...
        case cmd: InitScheduledStandardFlightsGeneration => scheduledFlightGeneration(context, timers, cmd.scheduledCmd, cmd.delay)
        case cmd: InitScheduledOverbookingFlightsGeneration => scheduledFlightGeneration(context, timers, cmd.scheduledCmd, cmd.delay)
  ...
  private def scheduledFlightGeneration(context: ActorContext[Command],
                                        ...
                                        cmd: GenerateFlights,
                                        delay: FiniteDuration): Behavior[Command] = {
